...
export const TenantProvidersEmployeesSection = ({
    purchasesEnabled,
    onTogglePurchases,
    employeesEnabled,
    onToggleEmployees,
    ...
    <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5">
        <ToggleRow
            icon={<Truck size={17} className="text-slate-500" />}
            title="Proveedores"
            description="Habilita el registro de proveedores y compras para este cliente."
            enabled={purchasesEnabled}
            onToggle={onTogglePurchases}
        />
        <div className="border-t border-slate-100" />
        <ToggleRow
            icon={<UserRound size={17} className="text-slate-500" />}
            title="Empleados"
